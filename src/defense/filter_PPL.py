...
class PPLDefenseFilter:
    ...
    def predict(self, chunk_text: str, query_text: str | None = None) -> tuple[bool, float]:  # noqa: ARG002
        """
    ...
    @classmethod
    def for_phase_a(cls, config: "ExperimentConfig", conn=None) -> "PPLDefenseFilter":
        """Defense A (pre-injection). conn accepted for API parity with ConsistencyDefenseFilter."""
        return cls.from_config(config, "pre_injection")

    @classmethod
    def for_phase_b(cls, config: "ExperimentConfig", conn=None) -> "PPLDefenseFilter":
        """Defense B (post-retrieval). conn accepted for API parity with ConsistencyDefenseFilter."""
        return cls.from_config(config, "post_retrieval")
